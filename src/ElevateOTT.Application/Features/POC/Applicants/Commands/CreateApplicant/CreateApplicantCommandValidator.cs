namespace ElevateOTT.Application.Features.POC.Applicants.Commands.CreateApplicant;
...
public class CreateApplicantCommandValidator : AbstractValidator<CreateApplicantCommand>
{
    ...
    public CreateApplicantCommandValidator()
    {
        ...
        RuleFor(a => a.Bmi).Cascade(CascadeMode.Stop)
            .InclusiveBetween(18.5, 24.9).WithMessage(Resource.Only_those_whose_BMI_between_18_5_and_24_9_are_allowed_for_enlisting);

        RuleForEach(a => a.ReferenceItems).SetValidator(new ReferenceItemForAddValidator());
    }
