...
public partial class CategoriesPage : ComponentBase, IAsyncDisposable
{
    ...
    private async Task DeleteCategory(Guid id)
    {
        ...
        if (!result.Cancelled)
        {
            var httpResponseWrapper = await CategoriesClient.DeleteCategory(id);
    ...
    private async Task<TableData<CategoryItem>> ServerReload(TableState state)
    {
        ...
        GetCategoriesQuery.PageSize = state.PageSize;
        ...
        {
            var successResult = responseWrapper.Response as SuccessResult<CategoriesResponse>;
            if (successResult != null)
                CategoriesResponse = successResult.Result;

            tableData = new TableData<CategoryItem>()
            { TotalItems = CategoriesResponse.Categories.TotalRows, Items = CategoriesResponse.Categories.Items };
        }
        else
        {
